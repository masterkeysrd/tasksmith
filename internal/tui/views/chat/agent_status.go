...
// AgentRunningStatus renders the active thinking/streaming status.
var AgentRunningStatus = kitex.FC("AgentRunningStatus", func(props AgentStatusProps) kitex.Node {
	t := theme.UseTheme()
	if t == nil {
		return nil
	}
...
// AgentCompletedStatus renders the final checkmark completed status.
var AgentCompletedStatus = kitex.FC("AgentCompletedStatus", func(props AgentStatusProps) kitex.Node {
	t := theme.UseTheme()
	if t == nil {
		return nil
	}
...
var AgentStatus = kitex.FC("AgentStatus", func(props AgentStatusProps) kitex.Node {
	if props.Sending {
		return AgentRunningStatus(props)
	}

	if props.LastFinishedTime >= 0 {
		return AgentCompletedStatus(props)
	}
